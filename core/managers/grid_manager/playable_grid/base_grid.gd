...
func _ready():
	# Get the game_manager
	#assert(get_parent().game_manager is GameManager)
	#game_manager = get_parent().game_manager
	
	# Needs to define things for the AStar
	# Needs to be +1,+1
	_astar.region = Rect2i(0, 0, 13, 9)
	_astar.cell_size = CELL_SIZE
	_astar.offset = CELL_SIZE * 0.5
	
	# define the compute and estimate algorithms
...
# Basic function to account for the offset on the canvas
func normalize_point_to_grid(pos: Vector2i) -> Vector2i:
	return pos - Vector2i(position)

# Returns coordinates for a specific position in the grid w/in (13,9) (WHAT?)
# Should be used to return two sets of coordinates -- the initial position, and the final position
...
# Basic function that determines if the position (already normalized) 
# is a walkable position
...
func is_melee_droppable(pos: Vector2i) -> bool:
	...
	if get_cell_source_id(0, _pos) == tiles.melee:
		return true
...
func is_ranged_droppable(pos: Vector2i) -> bool:
	...
	if get_cell_source_id(0, _pos) == tiles.ranged:
		return true
